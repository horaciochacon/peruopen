...
cache_info <- function() {
  ...
  catalog_info <- list(
    ...
    size_mb = if (file.exists(catalog_file)) round(file.size(catalog_file) / 1024 / 1024, 2) else 0,
    age_hours = if (file.exists(catalog_file)) round(as.numeric(difftime(Sys.time(), file.info(catalog_file)$mtime, units = "hours")), 1) else NA
  )
