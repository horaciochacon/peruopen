...
#' @param encoding Character. Encoding to use: "UTF-8", "Latin1", "Windows-1252",
#'   "ISO-8859-1", or "auto" (default "UTF-8")
      ...
      result <- if (format_lower %in% c("csv", "text/csv")) {
        ...
        if (encoding_to_use %in% c("Windows-1252", "ISO-8859-1", "Latin1")) {
          ...
          readr::read_csv(file_path,
            locale = readr::locale(encoding = encoding_to_use),
    ...
    score <- tryCatch(
      {
        ...
        score <- (spanish_count * 10) + (place_count * 50) -
          (corruption_count * 20) - (bad_place_count * 30)
