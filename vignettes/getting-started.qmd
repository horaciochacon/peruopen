...
Use `po_explore()` to get organized views of the data catalog:

```{r}
# General exploration of all data
overview <- po_explore()

# Explore health-related data specifically
health_data <- po_explore("salud")

# See what organizations publish the most data
overview$by_organization

# Find the most common file formats
overview$by_format
```
...
excel_data <- po_search(formats = c("XLSX", "XLS"))
```
...
budget_data <- po_search(organizations = "MEF")

# Search for economic indicators
economic <- po_search("economico")
```

### Geographic and Census Data

```{r}
# Get census data from INEI
census_data <- po_search(organizations = "INEI", tags = "censo")

# Find geographic boundary data
boundaries <- po_search("limite", formats = "SHP")
```
